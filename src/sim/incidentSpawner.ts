...
export function spawnFromTemplates(state: GameState, rng: SeededRNG, dt: number): void {
  ...
  for (const def of INCIDENTS) {
    ...
    // Play audio alert so player notices new incidents even without watching the feed
    if (def.severity === 'CRIT') soundNotifications.playIncidentCRIT();
    else if (def.severity === 'WARN') soundNotifications.playIncidentWARN();
    else soundNotifications.playIncidentINFO();
    if (state.activeIncidents.length >= cfg.maxConcurrent) return;
  }
}
